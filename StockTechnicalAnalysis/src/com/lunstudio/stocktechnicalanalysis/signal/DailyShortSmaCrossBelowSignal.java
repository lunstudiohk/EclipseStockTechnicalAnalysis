...
public class DailyShortSmaCrossBelowSignal extends BearishSignal {

	private static final Logger logger = LogManager.getLogger();

	private List<Integer> shortSmaList = null;
	
	public DailyShortSmaCrossBelowSignal(StockEntity stock, String priceType, Map<Date,BigDecimal> refPriceDateMap, List<StockPriceVo> stockPriceVoList, Integer signalType) throws Exception {
	...
	public boolean isValid(StockSignalEntity signal, Integer tradeIndex) throws Exception {
		if( this.shortSmaList.indexOf(-1*tradeIndex) != -1 ) {
			if( signal.isEmpty() ) {
				return true;
			}
			...
			//SMA-Period
			if( SignalParameterValidator.isSmaAbovePeriodValid(stockPriceVoList, this.shortSmaList, signal, tradeIndex) ) {
				return true;
			}
	...
	public static String getSignalDesc(StockSignalEntity signal) {
		...
		buf.append(String.format("%s [賣出 - 跌穿10MA]: ", signal.getStockCode()));
		if( signal.getLowerPeriod() != null ) {
			buf.append(String.format("[升穿10MA多於 %s日] ", signal.getLowerPeriod()));
		}
	...
	public static List<String> getSignalShortDesc(StockSignalEntity signal) {
		...
		lists.add("跌穿10MA");
		if( signal.getLowerPeriod() != null ) {
			lists.add(String.format("升穿10MA多於 %s日", signal.getLowerPeriod()));
		}
